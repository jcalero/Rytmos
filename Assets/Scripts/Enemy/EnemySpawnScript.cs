...
public class EnemySpawnScript : MonoBehaviour, PeakListener {
	...
	void Update() {
		...
		if (timer >= audioLength) {
			...
			else if (!Game.isUnlockedVersion && Application.loadedLevelName == "Game" && trialFadeOutTimer < 2f) {
	...
	void init() {
		...
		if (!Game.isUnlockedVersion && Application.loadedLevelName == "Game")
			audioLength = AudioManager.audioLength < 60 ? AudioManager.audioLength : 60;
		else
